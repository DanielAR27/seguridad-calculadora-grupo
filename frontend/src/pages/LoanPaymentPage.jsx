...
import LoanPaymentForm from '../components/calculations/LoanPaymentForm';
import LoanPaymentResults from '../components/calculations/LoanPaymentResults';
...
function LoanPaymentPage() {
  ...
  const baseRoute = '/calculator/loan-payment';
  ...
  useEffect(() => {
    if (id) {
      ...
      // Restablecer estado al acceder a la ruta base sin ID
      setCalculation(null);
      setIsSaved(false);
    }
  }, [id, navigate]);
  ...
  const handleCalculate = async (formData) => {
    ...
    try {
      const response = await calculatorApi.calculateLoanOnly(formData);
      setCalculation(response.data);
      setIsSaved(false);

      // Si se estaba viendo un cálculo guardado, regresar a la ruta base
      if (id) {
        navigate(baseRoute);
      }
    } catch (err) {
  ...
  const handleSaveToHistory = async () => {
    ...
    try {
      const response = await calculatorApi.saveLoanCalculation(calculation.inputs);
    ...
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800">
          {isSaved ? 'Viendo Cálculo Guardado' : 'Pago de Préstamo (Mensual)'}
        </h2>
        {isSaved && (
          <button
